		...
		<div class="w-full h-8 flex justify-between items-center">
			...
			<Badge>
				<SharedIcon
					...
					:color="
						props.item.isFavorite ? 'text-[#d4af37]' : 'text-gray-500'
					"
				>
				</SharedIcon>
			</Badge>
			<Badge size="w-20" color="text-slate-800" background="bg-[#edf0f7]">
				...
				<span>
					{{ dataStore.getMemberCount(props.item.taskID).length }}/{{
						props.item.memberCount
					}}</span
				>
			</Badge>
		</div>
		<TaskDone v-if="props.statu === 'Орындалды'" :point="props.item.point">
		</TaskDone>
	</div>
</template>
